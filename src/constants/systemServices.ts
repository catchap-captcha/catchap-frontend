...
> = {
  ...
  disk: {
    icon: 'ph-hard-drives',
    label: '서버 저장공간',
    // ★설명이 「영상·자막·이미지를 쌓는 곳」이었는데 ★거짓이었다(0816 실측).
    //   영상·자막·이미지는 오브젝트 스토리지에 있고(MEDIA_STORAGE_BACKEND=object)
    //   이 카드는 그것을 ★전혀 재지 않는다. 실제로 재던 것은 「백엔드 파드가 우연히
    //   앉은 노드의 디스크」라, 파드가 2벌이라 새로고침마다 값이 튀었다(60.9%↔61.6%).
    //   이제 서버 전부를 보고 ★가장 많이 쓴 곳을 말한다.
    // ⚠️대수(4대)를 여기 박지 않는다 — 「6과목」과 같은 실수가 된다. 백엔드가 세어 보낸다.
    desc: '서버들 중 가장 많이 쓴 곳 — 여기가 차면 그 서버에 새 프로그램이 못 뜹니다',
    kind: 'backing',
  },
};
